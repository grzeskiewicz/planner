  ...
  render() {
    ...
    const renderMonth = calendar.map((week, index) => {
      let renderWeek = week.map((day, index2) => {
        const unactive=((day.date < new Date(today).setHours(0, 0, 0, 0))  && day.date.getDate() !== today.getDate()) || (day.date < new Date(today).setHours(0, 0, 0, 0) && day.date!==new Date(today).setHours(0, 0, 0, 0)) ? true: false) ;
        let className =
       unactive ? "unactive":'' + " " +
